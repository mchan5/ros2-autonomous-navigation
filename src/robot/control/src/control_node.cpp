...
ControlNode::ControlNode(): Node("control") {
  ...
  // 1. TUNING PARAMETERS
  // Lookahead: Smaller (0.5) = tighter cornering. Larger (1.0) = smoother driving.
...
void ControlNode::pathCallback(const nav_msgs::msg::Path::SharedPtr msg) {
  ...
  if (current_path_.poses.empty()) {
      // Case A: We had no path before. Definitely new.
      is_new_mission = true;
  } 
  else if (!msg->poses.empty()) {
      // Case B: Compare the End Points (Goal).
  ...
  // 1. Always update the path data (So we see the curve around the obstacle)
  current_path_ = *msg;
  path_received_ = true;

  // 2. Only Reset Alignment (Stop & Turn) if it's a brand new mission
  if (is_new_mission) {
      ...
      // The timerCallback will automatically find the nearest point on this new curve.
  }
}
...
void ControlNode::timerCallback() 
{
  ...
  // --- PHASE 1: INITIAL ALIGNMENT (One-time setup) ---
  // This rotates the robot to face the path BEFORE we start driving.
  // It prevents the "taking off sideways" issue.
  if (!aligned_to_end_) {
      // Look at a point 1.0m down the path (not the end, just "forward")
      // We find the first point ~1.0m away to orient ourselves.
  ...
  // --- PHASE 2: REGULATED CONTINUOUS DRIVING ---

  // A. Prune old points (Prevent looping back)
  // We remove points that are behind us or very close
  double min_dist = 1e9;
  int closest_i = -1;
  ...
  // C. Calculate Steering (Pure Pursuit)
  double dx = tx - rx;
  double dy = ty - ry;
  ...
  // Curvature = 2y / L^2
  // Using 1.0 multiplier for steering polarity based on your last message
  double curvature = 1.0 * (2.0 * local_y) / dist_sq; 
  
  // Base steering command
  cmd.angular.z = curvature * linear_speed_;

  // D. SMART SPEED REGULATION (The "Efficiency" Fix)
  // If we are turning hard, SLOW DOWN.
  // Formula: Speed = MaxSpeed / (1 + |Steering| * Factor)
  ...
  if (dist_to_goal < 0.3) {
      ...
      // Heuristic: If angular velocity is 1.0 rad/s, reduce speed by half.
